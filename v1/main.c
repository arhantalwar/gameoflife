...
int main() {
    ...
    for(size_t i = 0; i < MAX_BLOX; ++i) {
        blox_array[i].x = 500 + blox_width;
        blox_array[i].y = 500 + 0*28;
    ...
    while(!WindowShouldClose()) {
        ...
        for(size_t i = 0; i < MAX_BLOX; ++i) {
            DrawTexture(texture[blox_array[i].num], blox_array[i].x, blox_array[i].y, WHITE);
            blox_array[i].num = blox_array[i].num & blox_array[i+1].num;
            blox_array[i].num = blox_array[i].num ^ blox_array[i+1].num;
            if(blox_array[i].num < 16){
                blox_array[i].num++;
            }
        }
